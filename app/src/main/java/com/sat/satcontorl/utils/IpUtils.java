...
public class IpUtils {
	...
	public static String getNetmask(Context mContext) {
	...
	public static InetAddress getBroadcastAddress(Context context)
		 ...
		 * byte[] quads = new byte[4]; for (int k = 0; k < 4; k++) { quads[k] =
		 * (byte) ((broadcast >> k * 8) & 0xFF); }
		 * 
		 * return InetAddress.getByAddress(quads);
		 */
		String hostIP = getHostIP();
		String nteMask = getNetmask(context);

		GetBroadAddress getBroadAddress = new GetBroadAddress();
		getBroadAddress.setNetaddr(hostIP);
		getBroadAddress.setNetmask(nteMask);
		getBroadAddress.execCalc();
		return InetAddress.getByName(getBroadAddress.getNetbroadcastaddr());
	...
	public static void openWifiBrocast(Context context) {
		WifiManager wifiManager = (WifiManager) context
				.getSystemService(Context.WIFI_SERVICE);
